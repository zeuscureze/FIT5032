...
<template>
  <div class="json-lab">
    ...
    <section class="lab-section">
      ...
      <ul>
        <li
          ...
          :key="author.id"
          :class="{
            'highlight': highlightedAuthor === author.id
          }"
          @click="highlightedAuthor = author.id"
        >
    ...
    <section class="lab-section">
      ...
      <ul>
        <li v-for="hours in bookstores.openingHours" :key="hours">
          weekend open at {{ hours.open }}, close at {{ hours.close }}
        </li>
...
const christie = computed(() => {
  return authors.find((author) => author.name === "Agatha Christie")
})

const orwellWorks = computed(() => {
  const author = authors.find(author => author.name === "George Orwell");
  return author ? author.famousWorks : [];
});

const austenWorks = computed(() => {
  const author = authors.find(author => author.name === "Jane Austen");
  return author ? author.famousWorks : [];
});

const christieWorks = computed(() => {
  const author = authors.find(author => author.name === "Agatha Christie");
  return author ? author.famousWorks : [];
});
